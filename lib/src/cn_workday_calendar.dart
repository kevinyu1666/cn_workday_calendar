...
/// 中国大陆工作日历。
///
/// 应用可以用它判断某天是否应该计入标准工作日，避免把调休补班日误判为周末，
/// 或者把节假日误判为正常工作日。
final class CnWorkdayCalendar {
  ...
  /// 判断指定日期是否为工作日。
  ///
  /// 在考勤、薪酬、排班和日历系统里，工作日判断会影响应出勤天数和业务分类，
  /// 因此这里必须优先处理国务院调休数据，再回退到自然周末规则。
  ...
  /// 获取某一天完整的工作日判断结果。
  ///
  /// 单个布尔值不足以解释“为什么这天算工作日/休息日”。这个方法会返回调休、
  /// 周末、节假日名称等上下文，方便调用方给用户展示可追溯原因。
  ...
  /// 统计指定日期区间内的工作日数量。
  ///
  /// 月度汇总和排班统计通常需要计算应出勤天数。这里包含起止日期，符合用户
